...
func init() {
	var err error
	summary, err = template.New("summary").Parse(`Longest kill: {{with .GetLongestKill}}{{.Killer}} killed {{.Victim}} with {{.Weapon}} at {{.Distance}}m{{end}}`)
	if err != nil {
		panic(err)
	}
}
...
func Test_WebhookHandler(t *testing.T) {
	...
	t.Run("posts a summary from the template to Discord", func(t *testing.T) {
		scoreboard := NewMockScoreboard(t)
		notifier := NewMockDiscordNotifier(t)
		handler := web.NewWebhookHandler(scoreboard, notifier, summary)
		...
		scoreboard.EXPECT().Reset()
		notifier.EXPECT().PostMessage("**Leaderboard:**...").Return(nil)                                          // forward the incoming message
		notifier.EXPECT().PostMessage("Longest kill: jmhobbs killed Reader with Screwdriver at 420m").Return(nil) // from out teemplate
